...
class TestAuthorizationCodeAuth:
    ...
    def test_parse_response_url(self):
        code = AsyncAuthorizationCode.parse_response_url(
    ...
    def test_parse_response_url_state_mismatch_raises(self):
        with pytest.raises(AuthenticationError):
            AsyncAuthorizationCode.parse_response_url(
    ...
    @pytest.mark.anyio
    async def test_exchange_code_and_cache(self, httpx_mock: HTTPXMock):
        ...
        token_info = await auth.exchange_code("auth-code-1")
    ...
    @pytest.mark.anyio
    async def test_refreshes_expired_token(self, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
            url=auth_module.TOKEN_URL,
            json={
    ...
    @pytest.mark.anyio
    async def test_refresh_bootstrap_from_constructor(
    ...
    @pytest.mark.anyio
    async def test_refresh_keeps_previous_refresh_token(
        self, httpx_mock: HTTPXMock
    ):
        httpx_mock.add_response(
            url=auth_module.TOKEN_URL,
    ...
    @pytest.mark.anyio
    async def test_missing_refresh_token_on_exchange_raises(
        self, httpx_mock: HTTPXMock
    ):
        httpx_mock.add_response(
            url=auth_module.TOKEN_URL,
        ...
        with pytest.raises(AuthenticationError):
            await auth.exchange_code("auth-code-2")
        await auth.close()

    @pytest.mark.anyio
    async def test_get_access_token_without_bootstrap_raises(self):
        auth = AsyncAuthorizationCode(
            client_id="client-id",
            client_secret="client-secret",
            redirect_uri="http://127.0.0.1:8080/callback",
    ...
    def test_concurrent_refresh_fetches_once(self, httpx_mock: HTTPXMock):
        if inspect.iscoroutinefunction(
            AsyncAuthorizationCode.get_access_token
        ):
            pytest.skip(
