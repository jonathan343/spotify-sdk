...
class TestChapterServiceGet:
    ...
    @pytest.mark.anyio
    async def test_get_chapter(self, httpx_mock: HTTPXMock):
        ...
        async with AsyncSpotifyClient(access_token="test-token") as client:
            chapter = await client.chapters.get("456")
    ...
    @pytest.mark.anyio
    async def test_get_chapter_with_market(self, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
        ...
        async with AsyncSpotifyClient(access_token="test-token") as client:
            chapter = await client.chapters.get("456", market="US")
...
class TestChapterServiceGetSeveral:
    ...
    async def test_get_several_empty_list_raises_error(self):
        async with AsyncSpotifyClient(access_token="test-token") as client:
            with pytest.raises(ValueError, match="ids cannot be empty"):
                await client.chapters.get_several([])

    @pytest.mark.anyio
    async def test_get_several_chapters(self, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
        ...
        async with AsyncSpotifyClient(access_token="test-token") as client:
            chapters = await client.chapters.get_several(["456", "789"])
    ...
    async def test_get_several_with_market(self, httpx_mock: HTTPXMock):
        ...
        async with AsyncSpotifyClient(access_token="test-token") as client:
            chapters = await client.chapters.get_several(["456"], market="US")
