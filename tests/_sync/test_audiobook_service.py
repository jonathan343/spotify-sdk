...
class TestAudiobookServiceGetSeveral:
    def test_get_several_empty_list_raises_error(self):
        with SpotifyClient(access_token="test-token") as client:
            with pytest.raises(ValueError, match="ids cannot be empty"):
                client.audiobooks.get_several([])

    def test_get_several_audiobooks(self, httpx_mock: HTTPXMock):
        httpx_mock.add_response(
        ...
        with SpotifyClient(access_token="test-token") as client:
            audiobooks = client.audiobooks.get_several(["123", "456"])
    ...
    def test_get_several_with_market(self, httpx_mock: HTTPXMock):
        ...
        with SpotifyClient(access_token="test-token") as client:
            audiobooks = client.audiobooks.get_several(["123"], market="US")
